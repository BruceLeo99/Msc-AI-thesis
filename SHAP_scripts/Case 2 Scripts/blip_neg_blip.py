...
output_folder = f"/var/scratch/yyg760/SHAP_results/SHAP_blip_neg_blip"
...
test_data_blip_1p = f101.Food101Dataset(img_to_analyze1, transform='vgg16', target_transform='integer', load_captions=True, caption_type=caption_type)
test_data_blip_2p = f101.Food101Dataset(img_to_analyze2, transform='vgg16', target_transform='integer', load_captions=True, caption_type=caption_type)
test_data_blip_5p = f101.Food101Dataset(img_to_analyze3, transform='vgg16', target_transform='integer', load_captions=True, caption_type=caption_type)
test_data_blip_10p = f101.Food101Dataset(img_to_analyze4, transform='vgg16', target_transform='integer', load_captions=True, caption_type=caption_type)

print(f"Full dataset size: {len(test_data_full_blip)}")
print(f"Images to analyze: {len(test_data_blip_1p)}")
print(f"Images to analyze: {len(test_data_blip_2p)}")
print(f"Images to analyze: {len(test_data_blip_5p)}")
print(f"Images to analyze: {len(test_data_blip_10p)}")

print("Loading models...")

model_path_mPBN1_blip = torch.load(f"/var/scratch/yyg760/results_final/mPBN_1p_{caption_type}/mPBN_1p_{caption_type}_best.pth", map_location=device)
model_path_mPBN2_blip = torch.load(f"/var/scratch/yyg760/results_final/mPBN_2p_{caption_type}/mPBN_2p_{caption_type}_best.pth", map_location=device)
model_path_mPBN5_blip = torch.load(f"/var/scratch/yyg760/results_final/mPBN_5p_{caption_type}/mPBN_5p_{caption_type}_best.pth", map_location=device)
...
model_mPBN1_blip_config = model_path_mPBN1_blip['model_config']
model_mPBN2_blip_config = model_path_mPBN2_blip['model_config']
model_mPBN5_blip_config = model_path_mPBN5_blip['model_config']
model_mPBN10_blip_config = model_path_mPBN10_blip['model_config']

model_mPBN1_blip_state_dict = model_path_mPBN1_blip['state_dict']
model_mPBN2_blip_state_dict = model_path_mPBN2_blip['state_dict']
model_mPBN5_blip_state_dict = model_path_mPBN5_blip['state_dict']
model_mPBN10_blip_state_dict = model_path_mPBN10_blip['state_dict']

model_mPBN1_blip_new_state_dict = OrderedDict()
for k, v in model_mPBN1_blip_state_dict.items():
    if k.startswith('module.'):
        name = k[7:] 
    else:
        name = k
    model_mPBN1_blip_new_state_dict[name] = v

model_mPBN2_blip_new_state_dict = OrderedDict()
for k, v in model_mPBN2_blip_state_dict.items():
    if k.startswith('module.'):
        name = k[7:] 
    else:
        name = k
    model_mPBN2_blip_new_state_dict[name] = v

model_mPBN5_blip_new_state_dict = OrderedDict()
for k, v in model_mPBN5_blip_state_dict.items():
    if k.startswith('module.'):
        name = k[7:] 
    else:
        name = k
    model_mPBN5_blip_new_state_dict[name] = v
...
model_mPBN1_blip = VisualBertPPNet(num_prototypes_per_class=1, num_classes=num_classes)
model_mPBN2_blip = VisualBertPPNet(num_prototypes_per_class=2, num_classes=num_classes)
model_mPBN5_blip = VisualBertPPNet(num_prototypes_per_class=5, num_classes=num_classes)
model_mPBN10_blip = VisualBertPPNet(num_prototypes_per_class=10, num_classes=num_classes)


print("Loading model weights...")

model_mPBN1_blip.load_state_dict(model_mPBN1_blip_new_state_dict)
model_mPBN2_blip.load_state_dict(model_mPBN2_blip_new_state_dict)
model_mPBN5_blip.load_state_dict(model_mPBN5_blip_new_state_dict)
model_mPBN10_blip.load_state_dict(model_mPBN10_blip_new_state_dict)

model_mPBN1_blip = torch.nn.DataParallel(model_mPBN1_blip)
model_mPBN2_blip = torch.nn.DataParallel(model_mPBN2_blip)
model_mPBN5_blip = torch.nn.DataParallel(model_mPBN5_blip)
model_mPBN10_blip = torch.nn.DataParallel(model_mPBN10_blip)

models_1p_blip = {
    'mPBN_1p_blip': model_mPBN1_blip,
}

models_2p_blip = {
    'mPBN_2p_blip': model_mPBN2_blip,
}
models_5p_blip = {
    'mPBN_5p_blip': model_mPBN5_blip,
}
models_10p_blip = {
...
output_folder, results_summary = run(models_1p_blip, device, background_blip, test_data_blip_1p, output_folder, background_batch_size) 
print(f"Analysis completed for image IDs: {img_ids_to_analyze1}") 

output_folder, results_summary = run(models_2p_blip, device, background_blip, test_data_blip_2p, output_folder, background_batch_size) 
print(f"Analysis completed for image IDs: {img_ids_to_analyze2}") 

output_folder, results_summary = run(models_5p_blip, device, background_blip, test_data_blip_5p, output_folder, background_batch_size) 
print(f"Analysis completed for image IDs: {img_ids_to_analyze3}") 

output_folder, results_summary = run(models_10p_blip, device, background_blip, test_data_blip_10p, output_folder, background_batch_size) 
