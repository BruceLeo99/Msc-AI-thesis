...
class VisualBertProtoPNetWrapper(nn.Module):
    """Wrapper class for multimodal ProtoPNet to work with KernelSHAP"""
    def __init__(self, model, background_batch, device):
        super().__init__()
        self.model = model
        self.device = device
    ...
    def __call__(self, x):
        """
        Handle visual inputs for KernelSHAP (numpy arrays) and create multimodal format internally
        Args:
            x: NumPy array with flattened visual features
        """
        # KernelSHAP passes numpy arrays, convert to tensors
        if isinstance(x, np.ndarray):
            visual_embeds = torch.from_numpy(x).float().to(self.device)
        else:
            visual_embeds = x.to(self.device)
        
        # Reshape flattened input back to image format [B, C, H, W]
        if len(visual_embeds.shape) == 2:  # [B, C*H*W]
            batch_size = visual_embeds.shape[0]
            visual_embeds = visual_embeds.view(batch_size, 3, 224, 224)
        elif len(visual_embeds.shape) == 1:  # [C*H*W]
            visual_embeds = visual_embeds.view(1, 3, 224, 224)
...
def run_shap_analysis_for_mpbn(model, model_name, test_batch, test_image_id, test_label,
    ...
    try:
        ...
        try:
            ...
            wrapped_model = VisualBertProtoPNetWrapper(actual_model, background, device)
            
            print(f"Type of wrapped_model: {type(wrapped_model)}")
            
            # Extract and flatten visual components for KernelSHAP
            ...
            # Denormalize and flatten background images for KernelSHAP
            background_images_denorm = denormalize_image(background_images)
            background_flat = background_images_denorm.view(background_images_denorm.shape[0], -1).detach().cpu().numpy()
            print(f"Background flattened shape: {background_flat.shape}")
            
            # Create KernelSHAP explainer with visual components only
            explainer = shap.KernelExplainer(wrapped_model, background_flat)
            print(f"Using KernelSHAP for {model_name} with visual-only attribution")
            
            # Prepare test data - extract visual component, denormalize and flatten
            ...
            print(f"KernelSHAP values computed")
            print(f"KernelSHAP values type: {type(shap_values)}")
            
            # Process SHAP values (should be numpy arrays now since we're using KernelSHAP)
            if isinstance(shap_values, list):
                ...
                print(f"KernelSHAP values is not a list")
            ...
            plt.suptitle(f'KernelSHAP Analysis: {model_name} - Image {test_image_id} (Label: {label_value})\nWith actual captions', fontsize=14)
            plt.tight_layout()
    ...
    for img_idx in range(len(test_data_to_analyze)):
        ...
        for model_name, model in models.items():
            ...
            if model is None:
                ...
                # Run multimodal KernelSHAP analysis with full batch dictionary
