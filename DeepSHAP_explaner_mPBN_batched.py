...
def run_shap_analysis_for_mpbn_batched(model, model_name, test_batch, test_image_id, test_label,
                                      background, device, output_folder, batch_size=15, caption_type=None):
    """
    BATCHED VERSION: Process background samples in smaller batches to avoid GPU memory overflow
    """
    ...
    # Extract visual components for batched processing
    ...
    background_batches = []
    background_dict_batches = []
    
    for i in range(0, total_background_size, batch_size):
        end_idx = min(i + batch_size, total_background_size)
        
        # Create visual batch
        visual_batch = background_images[i:end_idx]
        background_batches.append(visual_batch)
        
        # Create corresponding dict batch for wrapper
        dict_batch = background[i:end_idx]
        background_dict_batches.append(dict_batch)
    ...
    for batch_idx, (visual_batch, dict_batch) in enumerate(zip(background_batches, background_dict_batches)):
        print(f"Processing background batch {batch_idx + 1}/{len(background_batches)} (size: {visual_batch.size(0)})")
        
        # Move current batch to device
        visual_batch = visual_batch.to(device)
        
        # Create wrapper for this batch
        wrapped_model = VisualBertProtoPNetWrapper(actual_model, dict_batch)
        wrapped_model.to(device)
        
        # Denormalize background images for SHAP
        background_images_denorm = denormalize_image(visual_batch)
        # Enable gradients for SHAP
        ...
        # Clear GPU memory
        del visual_batch, background_images_denorm, explainer, wrapped_model
...
def load_background_from_image_ids(image_ids, dataset_type='val', transform='vgg16', target_transform='integer', load_captions=True, caption_type=None):
    """
    ...
    Args:
        ...
        caption_type: 'blip' or 'blip2' - which caption type to load
    Returns:
    ...
    background_dataset = f101.Food101Dataset(filtered_data, transform=transform, target_transform=target_transform, load_captions=load_captions, caption_type=caption_type)
    
    return background_dataset

def run(models, device, background, test_data_to_analyze, output_folder, background_batch_size=15, caption_type=None):
    """
    ...
    for img_idx in range(len(test_data_to_analyze)):
        ...
        for model_name, model in models.items():
            ...
            if model is None:
                ...
                success, result = run_shap_analysis_for_mpbn_batched(
                    model=model,
...
if __name__ == "__main__":
    ...
    # Prepare background data using actual dataset items with captions for batched processing
    print("Collecting background data...")
    background_data = []
    max_background_samples = 202  # Increased for better SHAP precision with batched processing
    ...
    output_folder = f"speedTest_mPBN_batched_202BG"
    os.makedirs(output_folder, exist_ok=True)
